...
void trim(char *str)
{ // removes excessive spaces before and after the string
    char *start = str;
    char *end;
...
void uart_comm(void *)
{
    ...
    while (1)
    {
        ...
        {
            if (data == '\n' || data == '\r')
            { // if we press enter, it will store the previous bytes
                if (index > 0)
                {
                    buffer[index] = '\0';
                    ...
                    else if (strcmp("replace", command) == 0)
                    {
                        ...
                        if (!temp)
                        {
                            ESP_LOGE(TAG, "Error opening file. TEMP_FILE");
                            return;
                        }
                        ...
                        if (replaced)
                        {
                            ESP_LOGE( TAG, "UID was replaced successfully.");
                        }
                        else
                        {
                            ESP_LOGE(TAG, "UID: %s was not found.", uid1);
                        }
                    }
